...
fun LocalBridgeNavHost(
    ...
    NavHost(
        ...
        composable(AppRoute.Home.route) {
            ...
            val pendingShare by container.externalShareCoordinator.pendingShare.collectAsStateWithLifecycle(
                initialValue = null
            )
            HomeScreen(
                uiState = uiState,
                onOpenDevices = { onNavigateToTopLevel(AppRoute.Devices.route) },
                onOpenChat = { onNavigateToTopLevel(AppRoute.Chat.route) },
                onOpenTransfers = { onNavigateToTopLevel(AppRoute.Transfers.route) },
                autoLaunchQrScanner = pendingShare?.shouldLaunchQrScanner == true &&
                    !uiState.connectionState.isConnected,
                onAutoLaunchQrScannerConsumed = container.externalShareCoordinator::markScannerPresented,
                onConnectQrPeer = { peer, pairingToken ->
                    if (pendingShare != null) {
                        container.externalShareCoordinator.markConnecting()
                    }
                    container.connectionService.connect(peer, pairingToken)
                }
            )
        }
        ...
        composable(AppRoute.Settings.route) {
            ...
            SettingsScreen(
                ...
                onBackToHome = { onNavigateToTopLevel(AppRoute.Home.route) },
