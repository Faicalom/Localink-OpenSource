...
fun LocalBridgeAndroidApp() {
    ...
    val pendingShare by application.container.externalShareCoordinator.pendingShare.collectAsStateWithLifecycle(
        initialValue = null
    )
    val connectionState by application.container.connectionService.connectionState.collectAsStateWithLifecycle(
        initialValue = application.container.connectionService.connectionState.value
    )
    ...
    LaunchedEffect(pendingShare?.id, connectionState.isConnected, currentRoute) {
        val shareRequest = pendingShare ?: return@LaunchedEffect
        if (connectionState.isConnected) {
            application.container.externalShareCoordinator.markSending()
            application.container.fileTransferService.queueFiles(shareRequest.uris)
            application.container.externalShareCoordinator.clear()
            if (currentRoute != AppRoute.Transfers.route) {
                navController.navigate(AppRoute.Transfers.route) {
        ...
        } else if (shareRequest.shouldLaunchQrScanner && currentRoute != AppRoute.Home.route) {
