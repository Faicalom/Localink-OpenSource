...
class LanFileTransferService(
    ...
    private var pendingForegroundStopJob: Job? = null
    ...
    override fun queueFiles(uris: List<Uri>) {
        scope.launch {
            ...
            val selectedFiles = resolveSelectedEntries(uris)
            if (selectedFiles.isEmpty()) {
                loggerService.warning("Android share intent did not expose any readable files.")
                return@launch
            }
    ...
    private fun resolveSelectedEntries(uris: List<Uri>): List<SelectedFile> {
        val resolvedFiles = mutableListOf<SelectedFile>()

        uris.distinct().forEach { uri ->
            val document = resolveSharedDocument(uri)
            if (document?.isDirectory == true) {
                val folderName = sanitizeFileName(
                    document.name?.takeIf { it.isNotBlank() } ?: "shared-folder"
                )
                val expandedFiles = collectDirectoryFiles(document, folderName)
                if (expandedFiles.isEmpty()) {
                    loggerService.warning("[SHARE] Shared folder $folderName did not expose any readable files.")
                } else {
                    loggerService.info("[SHARE] Expanded shared folder $folderName into ${expandedFiles.size} file(s).")
                    resolvedFiles += expandedFiles
                }
            } else {
    ...
    private fun resolveSharedDocument(uri: Uri): DocumentFile? {
        return runCatching {
    ...
    private fun collectDirectoryFiles(
        ...
        children.forEach { child ->
            val childName = sanitizeFileName(
                child.name?.takeIf { it.isNotBlank() } ?: "shared-item"
            )
            val nestedPath = "$currentPath/$childName"
            if (child.isDirectory) {
                resolvedFiles += collectDirectoryFiles(child, nestedPath)
            } else {
                resolveSelectedFile(
                    uri = child.uri,
                    fallbackName = nestedPath,
                    fallbackSize = child.length().takeIf { it >= 0L }
                )?.let(resolvedFiles::add)
            }
        }
    ...
    private fun syncForegroundTransferProtection(items: List<TransferItem>) {
        ...
        if (activeTransfers.isEmpty()) {
            pendingForegroundStopJob?.cancel()
            pendingForegroundStopJob = scope.launch {
                delay(1_500L)
        ...
        val detail = buildString {
            ...
            append(
                if (primaryTransfer.peerId.startsWith("bt-", ignoreCase = true)) {
                    " over Bluetooth"
                } else {
                    " over hotspot/LAN"
                }
