...
fun PairingQrScannerDialog(
    ...
    val boundCamera = remember { AtomicReference<androidx.camera.core.Camera?>(null) }
    val scanner = remember {
        val zoomCallback = ZoomSuggestionOptions.ZoomCallback { requestedZoomRatio ->
            val camera = boundCamera.get() ?: return@ZoomCallback false
            val maxSupportedZoomRatio = camera.cameraInfo.zoomState.value?.maxZoomRatio ?: requestedZoomRatio
            camera.cameraControl.setZoomRatio(requestedZoomRatio.coerceAtMost(maxSupportedZoomRatio))
            true
        }
