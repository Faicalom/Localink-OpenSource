...
import com.localink.android.MainActivity
import com.localink.android.R
...
class TransferForegroundService : Service() {
    ...
    private fun acquireLocks() {
        if (wakeLock?.isHeld != true) {
            ...
            wakeLock = powerManager
                ?.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "$packageName:localink-transfer")
                ?.apply {
        ...
        if (wifiLock?.isHeld != true) {
            ...
            wifiLock = wifiManager
                ?.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "$packageName:localink-transfer")
                ?.apply {
                    setReferenceCounted(false)
                    acquire()
                }
    ...
    companion object {
        private const val actionStartOrUpdate = "com.localink.android.action.TRANSFER_FOREGROUND_START_OR_UPDATE"
        private const val actionStop = "com.localink.android.action.TRANSFER_FOREGROUND_STOP"
