        ...
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(pairingToken))
            {
                await UpdateStateAsync(connectionState, state =>
                {
                    ...
                    state.StatusText = $"Enter the Localink confirmation code before connecting to {peer.DisplayName}.";
            ...
            await UpdateStateAsync(connectionState, state =>
            {
                ...
                state.HandshakeSummary = "Waiting for the Bluetooth socket before Localink pairing starts.";
                state.LastFailure = string.Empty;
            });
            ...
            if (client is null)
            {
                await FailAsync(
                    connectionState,
                    $"Bluetooth socket to {peer.DisplayName} could not be opened. Pair the devices in Windows Bluetooth settings first if needed, then use the Localink six-digit code inside the app. {connectFailure}");
                return;
            }
            ...
            await UpdateStateAsync(connectionState, state =>
            {
                ...
                state.HandshakeSummary = $"RFCOMM is live and Localink session {session.SessionId} is awaiting heartbeat validation.";
                state.LastFailure = string.Empty;
        ...
        if (pairingCode is null || !string.Equals(pairingCode, _sharedPairingCode, StringComparison.Ordinal))
        {
            ...
            await SendErrorEnvelopeAsync(
                ...
                errorMessage: errorCode == ProtocolErrorCodes.PairingTokenRequired
                    ? "The Localink confirmation code is required for Bluetooth pairing."
